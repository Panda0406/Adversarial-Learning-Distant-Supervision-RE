...
def load_data(ftrain, frela, fvec, args, data_path):
    ...
    GAN_relations, rela2sents, sent2sents = process_data(instances, data_path, label2id)
    repetitive_tripples = find_repetitive_tripples(rela2id)

    return instances, inputs, label2id, W, GAN_relations, rela2sents, sent2sents, repetitive_tripples

def process_data(instances, data_path, label2id):
    """
    Generate GAN training dataset for each relation that 
    ...
    for k, v in rela2sents.items():
        if len(v) > 1500 and k != 0:
            GAN_relations.append(k)
            print k, len(v)
    pickle.dump(GAN_relations, open(data_path + "GAN_relations.pkl", "wb"))
    pickle.dump(rela2sents, open(data_path + "rela2sents.pkl", "wb"))
    print 'There are %d relations in the training data' % (len(rela2sents))
    print '%d relations will be processed by this system, they are %s' % (len(GAN_relations), str(GAN_relations))

    return GAN_relations, rela2sents, sent2sents
